...
import { 
  ...
  FaWallet, FaHistory, FaCog, FaShieldAlt
} from "react-icons/fa";
    ...
    <nav className="bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900 shadow-xl sticky top-0 z-50 border-b border-blue-800/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 md:h-20">
          ...
          {/* Logo, Brand & Total Balance Alignment Group */}
            ...
            <Link to="/home" className="flex items-center space-x-3 group">
              ...
              <div className="flex flex-col">
                <span className="text-xl md:text-2xl font-bold text-white tracking-tight">
                  Student<span className="text-yellow-400">Bank</span>
                </span>
                <span className="text-[10px] md:text-xs text-blue-300 -mt-1">Premium Banking</span>
              </div>
            </Link>

            {/* Total Balance Moved Leftward (Desktop View) */}
            {isAuthenticated && (
              <div className="hidden sm:block px-4 py-1.5 bg-gradient-to-r from-yellow-400/10 to-yellow-500/10 rounded-full border border-yellow-400/20 backdrop-blur-sm">
                <div className="flex items-center space-x-2">
                  <FaWallet className="h-3.5 w-3.5 text-yellow-400" />
                  <div className="flex flex-col">
                    <span className="text-[9px] text-blue-300 uppercase font-semibold tracking-wider">Balance</span>
            ...
            {isAuthenticated ? (
              <div className="relative ml-4">
                <button
                  ...
                  className="flex items-center space-x-3 hover:opacity-95 transition"
                >
                  <div className="text-right">
                    <p className="text-xs text-blue-300 leading-none">Welcome back,</p>
                    <p className="text-sm font-semibold text-white mt-0.5">{user?.fullName?.split(' ')[0] || 'User'}</p>
                  </div>
                ...
                {showUserMenu && (
                  <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden animate-fade-in">
                    ...
                    <div className="p-2">
                      <DropdownItem icon={<FaUser />} label="My Profile" onClick={() => { navigate("/profile"); setShowUserMenu(false); }} />
                      <DropdownItem icon={<FaHistory />} label="History" onClick={() => { navigate("/history"); setShowUserMenu(false); }} />
                      <DropdownItem icon={<FaCog />} label="Settings" onClick={() => { navigate("/settings"); setShowUserMenu(false); }} />
                      <DropdownItem icon={<FaShieldAlt />} label="Security" onClick={() => { navigate("/security"); setShowUserMenu(false); }} />
                ...
                <div className="bg-gradient-to-r from-blue-600/20 to-indigo-600/20 rounded-xl p-4 mb-4 border border-blue-700/30">
                  ...
                  <div className="mt-3 pt-3 border-t border-blue-700/30">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-white font-bold text-lg font-mono">৳ {user?.balance?.toLocaleString()}</span>
                    </div>
                  </div>
                </div>

                {/* Simplified Mobile Links */}
                <MobileNavLink to="/profile" icon={<FaUser />} label="My Profile" onClick={() => setIsOpen(false)} />
                <MobileNavLink to="/history" icon={<FaHistory />} label="History" onClick={() => setIsOpen(false)} />
...
/* Dropdown Sub-Item styling component */
...
/* Mobile Sub-Navigation item row layout styling */
